...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      const distance = WEDDING_CONFIG.events.akad.startDateTime.getTime() - new Date().getTime();
      if (distance > 0) {
        setTimeLeft({
      ...
      <div className="z-10 container mx-auto px-6 flex flex-col items-center text-center">
        <div className="animate-reveal [animation-delay:200ms] space-y-4 md:space-y-10 w-full">
          <div className="flex items-center justify-center gap-3 md:gap-4">
            ...
            <span className="text-white/80 tracking-luxury text-[8px] md:text-[12px] uppercase font-light">The Wedding Celebration</span>
            <div className="h-[1px] w-6 md:w-20 bg-white/30"></div>
          </div>
          ...
          <div className="space-y-3 md:space-y-6">
            ...
            <div className="flex items-center justify-center gap-3 md:gap-4">
               ...
               <p className="text-accent/70 text-[9px] md:text-[13px] uppercase tracking-widest font-medium">Jakarta, Indonesia</p>
               <Sparkles className="w-3 h-3 md:w-4 md:h-4 text-accent/40 animate-pulse" />
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 md:mt-16 animate-reveal [animation-delay:600ms] flex items-center justify-center gap-4 md:gap-14 frosted-glass px-6 md:px-10 py-5 md:py-8 rounded-[1.5rem] md:rounded-[2.2rem] shadow-2xl border border-white/40 dark:border-white/10">
          {Object.entries(timeLeft).map(([label, value]) => (
            <div key={label} className="flex flex-col items-center min-w-[50px] md:min-w-[80px]">
        ...
        >
          ...
          <span className="text-[8px] md:text-[9px] uppercase tracking-luxury font-bold opacity-50 group-hover:opacity-100">Lihat Detail</span>
        </button>
      </div>
    </section>
  );
