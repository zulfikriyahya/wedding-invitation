...
import { Calendar, Clock, MapPin, ExternalLink, CalendarPlus, Copy, Check, ChevronDown, Sparkles, Heart } from 'lucide-react';
import { WEDDING_CONFIG } from '../constants';
import { generateGoogleCalendarUrl, downloadICS } from '../utils/calendarUtils';

const EventDetails: React.FC = () => {
  const [copied, setCopied] = useState(false);
  const [activeDropdown, setActiveDropdown] = useState<'akad' | 'resepsi' | null>(null);
  ...
  const handleCalendar = (type: 'google' | 'ics', eventType: 'akad' | 'resepsi') => {
    const event = {
  ...
  return (
    <section id="event" className="py-20 md:py-40 bg-secondary/30 dark:bg-darkBg transition-colors duration-1000 px-4 md:px-6">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16 md:mb-24 space-y-4 md:space-y-6">
          ...
          <h2 className="text-4xl md:text-9xl font-serif italic text-slate-900 dark:text-white tracking-tight">The Celebration</h2>
          <p className="text-slate-500 dark:text-slate-400 text-base md:text-2xl max-w-xl mx-auto italic font-light text-balance px-4">Merupakan kehormatan bagi kami jika Anda berkenan hadir.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 md:gap-14 mb-16 md:mb-20">
          {['akad', 'resepsi'].map((type) => {
            ...
            return (
              <div key={type} className="editorial-card relative bg-white dark:bg-darkSurface rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-20 flex flex-col items-center text-center space-y-8 md:space-y-12 group overflow-visible">
                <div className="absolute -top-4 -right-4 md:-top-6 md:-right-6 w-12 h-12 md:w-16 md:h-16 bg-accentDark/10 dark:bg-accent/10 rounded-full flex items-center justify-center text-accentDark dark:text-accent animate-float border border-white/20 dark:border-white/5 backdrop-blur-md z-10">
                   {type === 'akad' ? <Heart className="w-5 h-5 md:w-7 md:h-7 fill-current" /> : <Sparkles className="w-5 h-5 md:w-7 md:h-7" />}
                </div>
                
                <div className="space-y-2 md:space-y-4">
                  <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-luxury text-accentDark dark:text-accent">Our Sacred Day</span>
                  <h3 className="text-3xl md:text-7xl font-serif italic text-slate-900 dark:text-white leading-tight">{ev.title}</h3>
                </div>
                ...
                <div className="w-full relative">
                  <button
                    onClick={() => setActiveDropdown(activeDropdown === type ? null : type as any)}
                  ...
                  >
                    ...
                    Save The Date
                    <ChevronDown className={`w-3 h-3 md:w-4 md:h-4 transition-transform duration-500 ${activeDropdown === type ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  {activeDropdown === type && (
                    <div className="absolute top-full left-0 right-0 mt-3 md:mt-4 frosted-glass rounded-[1.5rem] md:rounded-[2rem] shadow-2xl z-[50] overflow-hidden animate-reveal p-2 md:p-3 border border-slate-200 dark:border-white/10">
                      ...
                      >
                        ...
                        <span className="font-bold text-[10px] md:text-[11px] uppercase tracking-luxury">Google Calendar</span>
                      </button>
                      <button
                      ...
                      >
                        ...
                        <span className="font-bold text-[10px] md:text-[11px] uppercase tracking-luxury">Apple / Outlook</span>
                      </button>
                    </div>
                  )}
                </div>
        ...
        <div className="editorial-card rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-20 space-y-12 md:space-y-16">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 md:gap-12">
            <div className="space-y-6">
              <div className="flex items-start md:items-center gap-5 md:gap-8">
                ...
                <div className="space-y-1 md:space-y-2">
                  <h4 className="font-serif italic text-3xl md:text-6xl text-slate-900 dark:text-white leading-tight tracking-tight">{WEDDING_CONFIG.venue.name}</h4>
                  <p className="text-slate-500 dark:text-slate-400 text-base md:text-2xl font-light italic leading-snug">{WEDDING_CONFIG.venue.address}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:gap-5 w-full lg:w-auto">
              ...
              >
                {copied ? <Check className="w-4 h-4 md:w-5 md:h-5 text-green-500" /> : <Copy className="w-4 h-4 md:w-5 md:h-5 text-accentDark dark:text-accent" />}
